...
global $object, $formadmin, $form, $langs;
...
foreach ($object->fields as $key => $val) {
	...
	if ($key === 'status') {
		...
		print '<select name="'.$key.'" id="'.$key.'" class="flat">';
		foreach ($statusOptions as $value => $label) {
			$selected = ($value === GETPOST($key)) ? 'selected' : '';
		...
		if (in_array($val['type'], array('int', 'integer'))) {
			...
			$check = !empty($tmparray[1]) ? $tmparray[1] : 'nohtml';
			$value = GETPOST($key, $check);
		} elseif (preg_match('/^html/', $val['type'])) {
			$tmparray = explode(':', $val['type']);
			$check = !empty($tmparray[1]) ? $tmparray[1] : 'restricthtml';
			$value = GETPOST($key, $check);
